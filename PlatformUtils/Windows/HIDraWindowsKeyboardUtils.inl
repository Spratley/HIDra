...
#define MAP_VK_KEY(VK, KEYCODE) case VK: return HIDra::KEYCODE
HIDra::KeyCode GetKeycodeFromWindowsVK(HIDra::HIDra_UInt16 windowsKeyCode, HIDra::HIDra_UInt16 flags)
{
    ...
    {
        return static_cast<HIDra::KeyCode>(HIDra::KEYCODE_A + (windowsKeyCode - 'A'));
    }

    if (windowsKeyCode >= '0' && windowsKeyCode <= '9')
    {
        return static_cast<HIDra::KeyCode>(HIDra::KEYCODE_0 + (windowsKeyCode - '0'));
    }

    if (windowsKeyCode >= VK_F1 && windowsKeyCode <= VK_F12)
    {
        return static_cast<HIDra::KeyCode>(HIDra::KEYCODE_F1 + (windowsKeyCode - VK_F1));
    }
    ...
    switch (windowsKeyCode)
    {
        ...
        case VK_CONTROL: return ((flags & RI_KEY_E0) != 0) ? HIDra::KEYCODE_CTRL_R : HIDra::KEYCODE_CTRL_L;
        case VK_MENU: // Alt
            return ((flags & RI_KEY_E0) != 0) ? HIDra::KEYCODE_ALT_R : HIDra::KEYCODE_ALT_L;

        default: return HIDra::KEYCODE_UNKNOWN;
    }
}

void ProcessVirtualKeystroke(HIDra::HIDra_UInt16 virtualKeyCode, HIDra::HIDra_UInt16 flags)
{
    using namespace HIDra;
    const bool pressed = (flags & RI_KEY_BREAK) == 0;
    KeyCode keyCode = GetKeycodeFromWindowsVK(virtualKeyCode, flags);
    KeyboardHandler& keyboardHandler = Core::GetInstance().GetKeyboardHandler();
    keyboardHandler.SetKeyState(keyCode, pressed);
}
